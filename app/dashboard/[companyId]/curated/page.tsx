...
export default async function CuratedPage({
	...
	// Get approved gigs
	const approvedGigs = await getCuratedGigs(companyId, "approved");
	const allGigs = await getCuratedGigs(companyId);
	const pendingCount = allGigs.filter((g) => g.status === "pending").length;
...
function EmptyState({ companyId }: { companyId: string }) {
	return (
		<div className="text-center py-16 px-8 bg-white/5 border border-white/10 border-dashed rounded-2xl">
			<div className="w-16 h-16 mx-auto mb-4 rounded-2xl bg-gradient-to-br from-emerald-500/20 to-emerald-500/10 flex items-center justify-center text-xl font-bold text-white/50">
				0
			</div>
			<h3 className="text-xl font-semibold text-white mb-2">
				No approved gigs yet
			</h3>
			<p className="text-white/50 mb-6 max-w-md mx-auto">
				Search for gigs and approve them to make them visible to your members.
			</p>
			<Link
